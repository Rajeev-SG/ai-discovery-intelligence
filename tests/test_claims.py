...
def test_real_capture_quotes_all_trace(source_id):
    ...
    if not path.exists():
        pytest.skip(f"capture {path.name} not present (deleted claim-spec flow retired)")
    raw = path.read_bytes()
...
def test_real_bundle_covers_the_three_required_proof_types():
    ...
    topics = {c["topic"] for c in expanded}
    source_ids = {c["source"]["source_id"] for c in expanded}
